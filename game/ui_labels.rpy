...
label gm_bare:
    ...
    python:
        ...
        ui.text(footerstring, text_align=0.5, xalign=0.5, yalign=0.98, size=33)
        ui.interact()
...
label confirm_quit:
    ...
    python:
        if not ask_to_quit:
            ...
            gi_result = _yesno_prompt(None, displayStrings.yesno_quit, im.Image("ui/sd-hanako.png",xpos=927,ypos=549), transition=transition)
            if gi_result:
                renpy.jump("softquit")
            else:
...
label confirm_mm:
    ...
    python:
        layout.navigation(None)
        if playthroughflag:
            gi_result = _yesno_prompt(None, displayStrings.yesno_return_to_main, im.Image("ui/sd-shizune.png",xpos=351,ypos=495))
            if gi_result:
                renpy.music.stop()
    ...
    python:
        ...
        if fn == "return":
            ...
            if not mm_context() and playthroughflag and statechangesincesave:
                if _yesno_prompt(None, displayStrings.yesno_load_in_game, im.Image("ui/sd-emi.png",xpos=918,ypos=495)):
                    renpy.load(fn)
            else:
                renpy.load(fn)
...
label extra_menu:
    ...
    python:
        ...
        ui.image(LiveComposite((1440, 1080),
                               (0, 0), style.mm_static.background,
                               (0, 0), style.gm_root.background,
                               (260, 180), "ui/bg-config.png"))
...
label act_op_ex(this_video, is_black=False):
    if not is_black:
        scene videowhite onlayer master
    elif True:
...
label music_menu_loop:
    ...
    python:
        ...
        ui.null(height=4)
        ui.hbox(xpos=4)
        vp = ui.viewport(yadjustment=yadj, mousewheel=False, draggable=False, xmaximum=730, ymaximum = 504, xalign=0.5, yalign=0.5)
        ui.vbox(xfill=True)
...
label scene_select_loop:
    ...
    python:
        ...
        vp = ui.viewport(yadjustment=yadj, mousewheel=False, draggable=False, xmaximum=720, ymaximum = 486, xalign=0.5, yalign=0.5)
        ui.vbox(xfill=True)
        for (name, label, display, path, unlocked) in available_scenes:
            if label == rp_actmark:
...
label cg_gallery:
    ...
    python:
        ...
        mygallery.background = (LiveComposite((1440, 1080),
                                              (0, 0), style.mm_static.background,
                                              (0, 0), style.gm_root.background,
                                              (260, 180), "ui/bg-config.png"))
        mygallery.locked_button = ib_disabled("ui/bt-cg-locked.png")
        mygallery.grid_layout((4,5),(315,288),(207,162))
        i = 0
        page = 0
...
label scene_deleted(is_end=False):
    ...
    scene expression prawn_image onlayer master:
        ...
        easein 0.3 zoom 1.0
        repeat
    show expression "ui/bt-logoonly.png" onlayer master:
...
label en_timeskip:
    ...
    if playthroughflag:
        ...
        show kslogo heart onlayer master at Position(xalign=0.5, yalign=0.5)
        with clockwipe

        scene black onlayer master
        show kslogo words onlayer master at Position(xalign=0.5, yalign=0.5)
        with clockwipe
...
label credits(c_movie=False):
    ...
    python:
        ...
        creditstext_disp = Text(displayStrings.creditstring, color="#ffffff", text_align=0.5)

        show_skipcredits_button = True

    scene videoblack onlayer master
    show credits mask onlayer master
    with Dissolve(2.0)

    show expression creditstext_disp as textsniff behind videoblack onlayer master
    with None
    ...
    show expression credits_final as roll behind credits onlayer master:
    ...
    if c_movie:
        play movie c_movie
        show expression Movie(fps=30, size=(400, 300), yalign=0.5, xanchor=0.5, xpos=0.3) as themovie behind roll onlayer master
    with None
...
label localized_tc(tc_text, auto=False):
    ...
    show videowhite onlayer master
    show expression Text("{b}{color=#908778}{size=36}"+tc_text+"{/size}{/color}{/b}", outlines=[]) as titlecard onlayer master:
