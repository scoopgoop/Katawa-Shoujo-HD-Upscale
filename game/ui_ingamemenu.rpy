init python:
    ...
    def ingamebutton(text, clicked, previously=None):
        ...
        img = Fixed(Text(text, xalign=0.5, yalign=0.5, color="#000"), xmaximum=1039)
        if previously:
            bgim = im.Composite(None,
                                (0,0), "ui/bg-choice.png",
                                (1039,2), im.Alpha("ui/bt-cf-checked.png",0.5))
        elif previously is not None:
            bgim = im.Composite(None,
                                (0,0), "ui/bg-choice.png",
                                (1039,2), im.Alpha("ui/bt-cf-unchecked.png",0.5))
        else:
            bgim = "ui/bg-choice.png"
        
        bg = Fixed(bgim, xmaximum=608)
        
        this_idle = LiveComposite((1039,63),
                                  (0,0), ingm_bg(bg),
                                  (0,0), ingm_btn(img)
                                  )
        this_hover = LiveComposite((1039,63),
                                  (0,0), ingm_bg(bg),
                                  (0,0), ingm_btn_hover(img)
                                  )
        
        ui.fixed(xpos=0.5, ymaximum=64, xmaximum=1440)
    ...
    def ingamebutton_dead(text, is_active=False):
        ...
        clicked = None
        if is_active:
            img = Fixed(Text(text, xalign=0.5, yalign=0.5, color="#ccc"), xmaximum=1039)
            bg = Fixed("ui/bg-choice_nochoice.png", xmaximum=608)
        else:
            img = Fixed(Text(text, xalign=0.5, yalign=0.5, color="#00000080"), xmaximum=1039)
