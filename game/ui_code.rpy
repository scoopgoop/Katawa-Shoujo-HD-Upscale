...
init 2 python:
    ...
    def return_button(wherefunc, text=None, xpos=975, ypos=800):
        if text == None:
            text = displayStrings.return_button_text
        ui.add(renpy.Keymap(game_menu=wherefunc))
        widget_button(text, "ui/bt-return.png", wherefunc, xsize=180, xpos=xpos, ypos=ypos)
    ...
    def indicator_icons():
        ...
        ui.text(' '.join(res), xanchor=1.0, xpos=1422, ypos=18 )
    ...
    class Gallery(object):
        ...
        def autobutton(self, in_images):
            ...
            if isinstance(in_images, tuple):
                ...
                in_images = (in_images,)
            if not thumbnail:
                thumbnail = im.Scale(ImageReference(thumb_base), 180, 135)
            button_base = LiveComposite((198, 153),
                                        (0, 0), ib_base("ui/bt-cg-locked.png"),
                                        (9, 9), im.MatrixColor(thumbnail, im.matrix.desaturate()))
